...
# Timing using all local (diffimTests) test code:
config = dit.ALdecMapReduceConfig()
# Timing for running it using the test version of the subtask
#  (dit.ALdecMapperSubtask2)
config = dit.ALdecMapReduceConfig2()
# Timing for running it using the stack-based version of the subtask
#  (ipDiffim.DecorrelateALKernelMapper)
config = ipDiffim.DecorrelateALKernelMapperConfig()
...
# Timing using all local (diffimTests) test code:
get_ipython().magic('timeit task.run(ALres.subtractedExposure, template=template2, science=exposure2,                   alTaskResult=ALres, forceEvenSized=True).exposure')
