...
print('Top', n, 'terms and counts:')
print()
for term_idx in sorted_term_idx_arr[:_____:_____]:
...
with open('target.txt') as f:
    push_score_sums = [_____ for line in f]

push_score_sum_s = pd.Series(push_score_sums)
push_score_sum_s.plot.hist(bins=_____)
...
with open('target.txt') as f:
    ...
    y = [_____ if _____ <= -1 else _____ for line in f]

from sklearn.model_selection import train_test_split

# doc: http://scikit-learn.org/stable/modules/generated/sklearn.model_selection.train_test_split.html
_____, _____, _____, _____ = train_test_split(_____, _____, test_size=0.3, random_state=0)
...
print('Traning Set Accuracy:', svc.score(_____, _____))
print('Validation Accuracy:', svc.score(_____, _____))
print()

print(classification_report(y_test, svc.predict(_____)))
...
for fidx in sorted_feature_idx_arr[:_____:_____]:
...
for fidx in sorted_feature_idx_arr[:_____]:
    print(feature_names[fidx], svc.coef_[class_i, fidx])
...
get_ipython().magic('time lsvc.fit(_____, _____)')
print()

print('Training Set Shape:', _____.shape)
print('Traning Set Accuracy:', lsvc.score(_____, _____))
print('Validation Accuracy:', lsvc.score(_____, _____))
print()

print(classification_report(_____, lsvc.predict(_____)))
...
X_train_2 = sfm.transform(_____)
X_test_2 = sfm.transform(_____)
...
get_ipython().magic('time lsvc_2.fit(_____, y_train)')
print()

print('Training Set Shape:', _____.shape)
print('Traning Set Accuracy:', lsvc_2.score(_____, y_train))
print('Validation Accuracy:', lsvc_2.score(_____, y_test))
print()

print(classification_report(_____, lsvc_2.predict(_____)))
...
get_ipython().magic('time gscv.fit(_____, _____)')
print()
...
get_ipython().magic('time best_lsvc.fit(_____, _____)')
print()

print('Traning Set Accuracy:', best_lsvc._____(_____, _____))
print('Validation Accuracy:', best_lsvc._____(_____, _____))
print()

print(_____(_____, best_lsvc._____(_____)))
