...
im1stats = np.array([t[0] for t in tmp]); im1stats[im1stats>70] = 70; print im1stats.min(), im1stats.max(), im1stats.std()
tmp = computeVarianceBoxStats(im2)
im2stats = np.array([t[0] for t in tmp]); im2stats[im2stats>135] = 125; print im2stats.min(), im2stats.max(), im2stats.std()
tmp = computeVarianceBoxStats(sciImg)
scistats = np.array([t[0] for t in tmp]); print np.sort(scistats)[4], np.sort(scistats)[-4], scistats.std()
tmp = computeVarianceBoxStats(templImg)
templstats = np.array([t[0] for t in tmp]); print np.sort(templstats)[4], np.sort(templstats)[-4], templstats.std()
...
corrKernels2 = [DecorrelateALKernelTask._computeDecorrelationKernel(kimg, svar=v2, tvar=v1) for     #xy in [(None, None)] for v1 in (58., None) for v2 in (64.5, None)]
    kimg in [kimages[coo[0]], kimages[coo[1]]] for v1 in (60., 58.9, 61.3) for v2 in (62.7, 61.5, 64.1)]
print np.max([kc.max() for kc in corrKernels2]), np.min([kc.max() for kc in corrKernels2])
print np.max([kc.min() for kc in corrKernels2]), np.min([kc.min() for kc in corrKernels2])
...
decorrResults = [doDecorr(sciImg, templImg, im1, spatialKernel, xcen=xy[0], ycen=xy[1], svar=v2, tvar=v1) for     #xy in [(None, None)] for v1 in (58., None) for v2 in (64.5, None)]
    xy in [(1023,2047), (0,4094), (2046,0), (None, None)] for v1 in (60., 58.9, 61.3, None) for v2 in (62.7, 61.5, 64.1, None)]
...
ns = np.array("""50 50 50 50 51 50 52 51 49 48 50 49 50 50 50 50 50 50 52 50 52 50 52 52 50
 48 50 50 50 50 52 50 49 49 50 49 51 50 51 51 48 47 49 48 49 49 50 49 50 50
 50 50 51 50 52 51 49 48 50 49 50 50 50 50""".split(' ')).astype(int)
print ns.mean(), ns.min(), ns.max(), ns.std(), ns.std()/ns.mean()
