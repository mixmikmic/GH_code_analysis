...
cols.append('price_full')

test['price_doc'] = np.nan
test['price_full'] = np.nan
...
df_all.drop(['timestamp','quarter'], axis=1, inplace=True)
...
df_sub = pd.DataFrame({'id': test_index, 
                       'price_doc': y_pred * df.loc[test_index,'full_sq'] * fudge_factor,
...
# 0.32046
df_sub[['price_doc']].to_csv('../submissions/sub_XGB_0530_10PM_fudge_98.csv')
