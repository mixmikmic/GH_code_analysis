...
# first run uses Using trilinear_interp, second uses direct summation so must be overwriting
#old = AAA.ds
#new = Dm.ds
#sum_dcm = SumPlanRC(old, new)  # uses sum

# first run uses Using trilinear_interp, second uses direct summation so must be overwriting, ONLY RUN ONCE
old = AAA.ds
new = Dm.ds
...
w = FloatSlider(
    value=0.56,
    min=0,
    max=diff_dcm.pixel_array.shape[0],   # get the z depth
...
def showdose(z):
    diff_image = diff_dcm.pixel_array[int(z),:,:]
    plt.imshow(diff_image)
    plt.title('Max pixel is  ' + str(diff_image.max()))
