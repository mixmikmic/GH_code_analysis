...
# Training on 1000 posters and predicting on 500 
# not possible to load all 7000+ training images and 3700+ testing images and run PCA on CPU

X_train_unshaped, Y_train = load_posters_from_ids(X_train_ids, 1000, y_train)
X_test_unshaped, Y_test = load_posters_from_ids(X_test_ids, 500, y_test)
...
# look at number of movies in each category in the test set
print 'train set num per genre ', np.sum(Y_train, axis=0)
print 'test set num per genre ', np.sum(Y_test, axis=0)

# look at number of movies in each category in the predictions
