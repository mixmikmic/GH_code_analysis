...
model_zero = sm.GLM(Y0.astype(int), exog = Z, family=sm.families.Binomial(link = linkclass),                    offset = offsetz , freq_weights = weights).fit()
...
while np.absolute((ll_old - ll_new)/ll_old) > reltol :
    ...
    model_zero = sm.GLM(probi, exog = Z, family=sm.families.Binomial(link = linkclass),                        offset = offsetz, freq_weights = weights,                         start_params = start['zero']).fit()
    start = {'zero':model_zero.params, 'count':model_count.params}
