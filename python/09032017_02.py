...
G0_node, X0, _= SBN.smooth_gsignal_filter(G0, option_sig, sigma=10/bias, nloop=2, seed=20, add_noise=False, show_plot=True, save_fig=savefigure)
...
fig2= plt.figure(2, figsize=(15,6))
ax = fig2.add_subplot(121)
cax = ax.matshow(sp.sign(abs(pywt.threshold(sparse_cvx_o-low_rank_cvx_o, 1e-3, 'hard'))), cmap=plt.cm.gray)
fig2.colorbar(cax)
ax = fig2.add_subplot(122)
...
plt.show()
filename = "../figures/"+strftime("%d%m%Y_%H%M%S", gmtime()) + "_precision_marginal_laplacian_em.eps"
if savefigure : fig2.savefig(filename)
