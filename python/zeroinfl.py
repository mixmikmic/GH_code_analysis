...
class ZeroInflated(object):
    ...
    def __init__(self, formula_str, data, dist = 'poisson', offsetx = None, offsetz = None,
        ...
        self.terms = {'Y':self.endog.columns.values[0],'X':self.X.columns.values,                      'Z':self.Z.columns.values}
    ...
    def gradPoisson(self, parms, sign = 1.0):
        ...
        wres_zero  = np.where(self.Y1,-1/(1-muz) * link_etaz,                           (link_etaz - np.exp(clogdens0) * link_etaz)/dens0)   
    
        return sign*(np.hstack((np.expand_dims(wres_count*self.weights,axis=1)*self.X,                 np.expand_dims(wres_zero*self.weights,axis=1)*self.Z))).sum(axis=0)
    ...
    def ziNegBin(self, parms, sign = 1.0):
        ...
        loglik0 = np.log(phi + np.exp(np.log(1-phi) +                                    st.nbinom.logpmf(0,*self.convert_params(theta = theta, mu = mu)) ) )
    ...
    def gradGeom(self, parms, sign = 1.0):
        ...
        ## working residuals  
        wres_count = np.where(self.Y1,self.Y - mu*(self.Y + 1)/(mu + 1),                               -np.exp(-np.log(dens0) + np.log(1 - muz) + clogdens0 +                                      -np.log(mu+1) + np.log(mu))) 
        link_etaz = self.linkobj.link_inv_deriv(etaz)
        wres_zero  = np.where(self.Y1,-1/(1-muz) * link_etaz,                           (link_etaz - np.exp(clogdens0) * link_etaz)/dens0)
    ...
    def gradNegBin(self, parms, sign = 1.0): 
        ...
        ## working residuals  
        wres_count = np.where(self.Y1,self.Y - mu*(self.Y + theta)/(mu + theta),                               -np.exp(-np.log(dens0) + np.log(1 - muz) + clogdens0 + np.log(theta)                                       -np.log(mu+theta) + np.log(mu))) 
        link_etaz = self.linkobj.link_inv_deriv(etaz)
        wres_zero  = np.where(self.Y1,-1/(1-muz) * link_etaz,                           (link_etaz - np.exp(clogdens0) * link_etaz)/dens0)
        
        wres_theta = theta*np.where(self.Y1, sp.special.digamma(self.Y + theta) - sp.special.digamma(theta) +                                   np.log(theta) - np.log(mu + theta) + 1 - (self.Y + theta)/(mu + theta),                                   np.exp(-np.log(dens0) + np.log(1 - muz) + clogdens0)*                                   (np.log(theta) - np.log(mu + theta) + 1 - theta/(mu+theta) ) )
        
        return sign*(np.hstack((np.expand_dims(wres_count*self.weights,axis=1)*self.X,                 np.expand_dims(wres_zero*self.weights,axis=1)*self.Z,                                np.expand_dims(wres_theta,axis=1)))).sum(axis=0)
    
    def EM_estimate(self):
        ## EM estimation of starting values
        
        model_count = sm.GLM(endog = self.Y, exog = self.X, family = sm.families.Poisson(),                                  offset = self.offsetx , freq_weights = self.weights).fit()
        model_zero = sm.GLM(self.Y0.astype(int), exog = self.Z, family=sm.families.Binomial(link = self.linkobj.linkclass),                    offset = self.offsetz , freq_weights = self.weights).fit()
        ...
        if (self.EM is True) and (self.dist is 'poisson'):
            ...
            while np.absolute((ll_old - ll_new)/ll_old) > self.reltol :
                ll_old = ll_new
                model_count = sm.GLM(endog = self.Y, exog = self.X, family = sm.families.Poisson(),                                  offset = self.offsetx , freq_weights = self.weights*(1-probi)                                               ).fit(start_params = self.start['count'].values)        
                model_zero = sm.GLM(probi, exog = self.Z, family=sm.families.Binomial(link = self.linkobj.linkclass),                        offset = self.offsetz, freq_weights = self.weights                                ).fit(start_params = self.start['zero'].values)
                self.start = {'zero':model_zero.params, 'count':model_count.params}
        ...
        if (self.EM is True) and (self.dist is 'geom'):
            ...
            while np.absolute((ll_old - ll_new)/ll_old) > self.reltol :
                ll_old = ll_new
                model_count = sm.GLM(endog = self.Y, exog = self.X, family = sm.families.NegativeBinomial(alpha = 1.0),                                  offset = self.offsetx , freq_weights = self.weights*(1-probi)).fit(                                        start_params = self.start['count'].values)
        ...
        if (self.EM is True) and (self.dist is 'negbin'):
            ...
            while np.absolute((ll_old - ll_new)/ll_old) > self.reltol :
                ll_old = ll_new
                model_count = sm.GLM(endog = self.Y, exog = self.X, family =                                 sm.families.NegativeBinomial(alpha = 1/self.start['theta']),method = 'newton',                                  offset = self.offsetx , freq_weights = self.weights*(1-probi)                                       ).fit(start_params = self.start['count'])
                model_zero = sm.GLM(probi, exog = self.Z, family=sm.families.Binomial(link = self.linkobj.linkclass),                        offset = self.offsetz, freq_weights = self.weights,                         start_params = self.start['zero']).fit()
    ...
    def fit(self, method = 'BFGS', EM = True, start = None, reltol = None,            options = {'disp': False, 'maxiter': 10000}, factr = 1.0):
        ...
        ## ML Estimation
        if (self.dist is 'negbin'):
            x0 = np.hstack((self.start['count'].values,self.start['zero'].values,                                         np.log(self.start['theta'])))
        else:
            x0 = np.hstack((self.start['count'].values,self.start['zero'].values))

        fitResult = sp.optimize.minimize(self.loglikfun, args=(-1.0,), x0 = x0,                                         method=self.method, jac=self.gradfun, options=self.optim_options)
        ...
        vc = pd.DataFrame(data = vc_data[np.arange(self.kx+self.kz)[:,None],np.arange(self.kx+self.kz)],                       index = np.append(self.X.columns.values, self.Z.columns.values),                 columns = np.append(self.X.columns.values, self.Z.columns.values))
        if self.dist == 'negbin':
        ...
        Result = ZeroInflatedResults(self.call, self.formula, self.terms, self.kx, self.kz,                                      self.dist, self.link, self.linkobj, self.optim_options, self.method, self.start,                                     self.reltol, self.weights, self.offsetx, self.offsetz,                                     fitResult, coefc, coefz, theta, SE_logtheta, nobs, res, Yhat, vc, self.endog,                                    self.X, self.Z)
        return Result
...
class ZeroInflatedResults(object):
    def __init__(self, call, formula, terms, kx, kz, dist, link, linkobj, options, method, start, reltol,                  weights, offsetx, offsetz, fitResult, coefc, coefz, theta, SE_logtheta,                nobs, res, Yhat, vc, y, X, Z):
    ...
    def printModel(self):
        ...
        c_df = pd.DataFrame(np.array(self.coefficients['count']),index = self.terms['X'], columns = list(" ")                           ).applymap("{0:.5f}".format)
        ...
        #z_df = pd.DataFrame(np.array(zero_coeff).reshape(len(zero_coeff),2), columns = list("  "))
        z_df = pd.DataFrame(np.array(self.coefficients['zero']),index = self.terms['Z'], columns = list(" ")                           ).applymap("{0:.5f}".format)
        #print (z_df.to_string())
        print(z_df.T)
    ...
    def summary(self):
        ...
        data_count = [coeff_label] + list(zip(self.terms['X'], np.round(self.coefficients['count'],4),                                         np.round(se[0:self.kx], 4), np.round(z_count, 3), pc_format))
        ...
        print(MODEL2_HEADER)
        data_zero = [coeff_label] + list(zip(self.terms['Z'], np.round(self.coefficients['zero'],4),                                         np.round(se[self.kx:], 4), np.round(z_zip, 3), pz_format))
...
ZeroInflated(formula_str, df, dist='negbin').fit(EM=False).summary()
