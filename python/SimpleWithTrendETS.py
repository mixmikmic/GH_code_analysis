...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if (epoch + 1) % display_step == 0:
            ...
            print "Epoch:", '%04d' % (epoch+1),                 "cost=", "{:.9f}".format(c),                 "level0=", sess.run(level0),                 "pace0=", sess.run(pace0),                 "alpha=", sess.run(alpha),                 "beta=", sess.run(beta)

    print "Optimization Finished!"
    training_cost = sess.run(cost, feed_dict={data_in: y, data_out: y})
    print "Training cost=", training_cost,         "level0=", sess.run(level0),         "pace0=", sess.run(pace0),         "alpha=", sess.run(alpha),         "beta=", sess.run(beta)
