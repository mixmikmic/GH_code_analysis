...
# We'll use tkinter to open a dialog box to select the data file to use
import tkinter
from tkinter import filedialog

# We'll use the smoothing and detrending functions in signal
...
dt = time[1] - time[0] # calculate the sampling time from the data

# Now, determine the indices of the start and end points of the part of the response 
# that we'd like to slice. We need int() in order to use the result as a index
# in the array slicing.
start_index = int(np.ceil(2.75 / dt))
end_index = int(np.floor(17 / dt))

# Now, actually slice the arrays.
time = time[start_index:end_index] - time[start_index] # We substract the time of the start_index to make t[0] = 0
vib_detrend = vib_detrend[start_index:end_index]
...
zeros = get_zero_crossings(time, vib_detrend)

# Get the peaks of the response
localMaxes, localMax_Times, localMins, localMin_Times = get_local_Extrema(time, vib_detrend)
...
plt.plot(time, vib_detrend, linewidth=2, linestyle="-", label=r'Response')
