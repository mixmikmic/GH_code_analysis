...
target_url = 'http://www.yelp.com/search?find_desc=Restaurants&find_loc=NewYork+NY&start='
...
biz_links = open('cleanbiz_ny_links.txt', 'w')
for item in link_dict.keys():
    ...
    biz_links.write("%s\n" % item)
biz_links.close()
print("\nFinished writing to file:  cleanbiz_ny_links.txt")

link_file = open("cleanbiz_ny_links.txt", "r")
...
with open('newyork_reviews.json', 'w') as outfile:
