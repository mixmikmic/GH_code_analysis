...
class NeuralNetwork(object):
    ...
    def train(self, inputs_list, targets_list):
        ...
        ### Forward pass ###
        # TODO: Hidden layer
        hidden_inputs = np.matmul(self.weights_input_to_hidden, inputs)
        ...
        hidden_errors = np.matmul(self.weights_hidden_to_output.T, output_errors) * hidden_outputs * (1 - hidden_outputs)
        hidden_grad = np.matmul(hidden_errors, inputs.T)
...
for e in range(epochs):
    if e >= 2000 and e % 100 == 0:
        network.lr = network.lr * np.power(0.95, (e - 1900) / 100)
        print('\tnew lr: ', network.lr)
