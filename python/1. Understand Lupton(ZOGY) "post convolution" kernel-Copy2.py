...
tmp = post_conv_kernel_ft.evalf(subs={m:0, sigk:1, sig1:1, sig2:1})
sym.inverse_fourier_transform(tmp, k, x)
...
plt.plot(np.fft.fftshift(freqs), np.fft.fftshift(abs(FFT)*(x[1]-x[0])))
plt.xlim(-3, 3)
...
pck = post_conv_kernel(x, sig2=10.)
xx = np.fft.ifftshift(x)
plt.plot(xx, pck.real)
plt.xlim(-0.3, 0.3)
...
post_conv_kernel(x, sig1=1., sig2=10., m=0., sigk=10.0)
xx = np.fft.ifftshift(x)
plt.plot(xx, pck.real)
plt.xlim(-0.3, 0.3)
