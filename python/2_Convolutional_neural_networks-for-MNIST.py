...
testSampleIndexes = [ int(random.random() * len(X_test)) for i in range(4)]
print("Test sample indexes: ",  testSampleIndexes )
testSamples = np.concatenate([np.concatenate([X_test[i].reshape(28,28) for i in testSampleIndexes], axis=1) for i in range(1)], axis=0)
plt.figure(figsize=(4,1))
plt.imshow(testSamples, cmap='gray')

ts_prob = [ model.predict( X_test[ testSampleIndexes[i] ].reshape(1,28,28,1) ) for i in range(4) ]
ts_pred = [ ts_prob[i].argmax()  for i in range(4) ]
ts_actual = [ y_test[ testSampleIndexes[i] ].argmax()  for i in range(4) ]

print("Predicted: ", ts_pred)
print("Actual:    ", ts_actual)
