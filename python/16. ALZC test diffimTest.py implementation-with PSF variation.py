...
im1, im2, im1_psf, im2_psf = dit.makeFakeImages(psf_yvary_factor=0.2, theta2=-45., im2background=10.)
...
dit.plotImageGrid((im1[x1d:x2d,y1d:y2d], im2[x1d:x2d,y1d:y2d], (im2-im1-10.)[x1d:x2d,y1d:y2d]*1000), 
                  extent=extent, imScale=3, clim=(-0.1, 1000), cbar=True, titles=['Template', 'Science', 'Raw diff'])
...
extent = (x0im.min()+150, x0im.max()-150, y0im.min()+150, y0im.max()-150)
x1d, x2d, y1d, y2d = 150, 512-150, 150, 512-150   # limits for display
dit.plotImageGrid((diffim1[x1d:x2d,y1d:y2d], diffim2[x1d:x2d,y1d:y2d]), imScale=4., clim=(-1,1), cbar=True,
                 titles=['A&L', 'ALZC'])
...
dit.plotImageGrid((pcf, im2_psf_small), clim=(0,0.01), cbar=True)
...
extent = (x0im.min()+150, x0im.max()-150, y0im.min()+150, y0im.max()-150)
x1d, x2d, y1d, y2d = 150, 512-150, 150, 512-150   # limits for display
dit.plotImageGrid((D[x1d:x2d,y1d:y2d]/D.std(), diffim1[x1d:x2d,y1d:y2d]/diffim1.std(), 
                   diffim2[x1d:x2d,y1d:y2d]/diffim2.std()), imScale=4., clim=(-1,1), cbar=True,
                 titles=['ZOGY', 'A&L', 'ALZC'])
...
dit.plotImageGrid(((diffim1-D)[x1d:x2d,y1d:y2d], (diffim2-D)[x1d:x2d,y1d:y2d]), imScale=4., clim=(-0.1,0.1),
                 cbar=True, titles=['A&L - ZOGY', 'ALZC - ZOGY'])
...
dit.plotImageGrid((cov1, cov2, cov3), imScale=4., clim=(0, 0.1), cmap=None, cbar=True,
                 titles=['A&L', 'ALZC', 'ZOGY'])
#plt.colorbar()
