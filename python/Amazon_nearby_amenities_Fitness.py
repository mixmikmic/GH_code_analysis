...
fitness_places = {'gym':'gym'}
all_fitness_rating = []

for key in fitness_places.keys():
    fitness_rating = []
    for site in places_df.values:

        # geocoordinates
        target_coordinates = str(site[3]) + ',' + str(site[4])
        target_search = fitness_places[key]
        target_radius = 2500
        target_type = key
        print("For {} at {}: {}, {}".format(key, site[0], site[1], target_coordinates))
        print("----------")
        ...
        for x in range(total_counts):
            if "rating" in results[x].keys():
                ...
                rating = 'NAN'
            fitness_rating.append({"Site Name":site[2],
            ...
            time.sleep(3)
        time.sleep(3)
    all_fitness_rating.append(fitness_rating)
    print("ALL Done!!!!!")
#all_fitness_rating

all_fitness_rating

all_gym_rating_df = pd.DataFrame(all_fitness_rating[0])
all_gym_rating_df



all_gym_rating_df.to_csv("Gym_Rating.csv")
