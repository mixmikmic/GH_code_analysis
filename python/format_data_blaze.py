...
train_X.domain = train_X.domain.apply(lambda x: 'NULL' if x == 'null' else x)
valid_X.domain= valid_X.domain.apply(lambda x: 'NULL' if x == 'null' else x)
test_X.domain= test_X.domain.apply(lambda x: 'NULL' if x == 'null' else x)

train_X.fillna('NULL', inplace=True)
valid_X.fillna('NULL', inplace=True)
test_X.fillna('NULL', inplace=True)
...
sss= set()
for i in train_X[train_X.usertag.notnull()].usertag:
    sss |= set(i)
print("there are {} usertags in total train set".format(len(sss)))

ttt= set()
for i in valid_X[valid_X.usertag.notnull()].usertag:
    ttt |= set(i)
print("there are {} usertags in total valid set".format(len(ttt)))

uuu= set()
for i in test_X[test_X.usertag.notnull()].usertag:
    uuu |= set(i)
...
valid_X.to_pickle(os.path.join(DATA_DIR,'valid_X_blaze'))
test_X.to_pickle(os.path.join(DATA_DIR,'test_X_blaze'))

train_y.to_pickle(os.path.join(DATA_DIR,'train_y_blaze'))
valid_y.to_pickle(os.path.join(DATA_DIR,'valid_y_blaze'))
...
train_X_1.to_pickle(os.path.join(DATA_DIR,'train_X_1_blaze'))
train_X_2.to_pickle(os.path.join(DATA_DIR,'train_X_2_blaze'))
