...
#plotted histogram
plt.hist(dataset_velo[0][:,0])
plt.hist(dataset_velo[0][:,1])
plt.hist(dataset_velo[0][:,2])
...
# Submissions - 3d cloud point on camera
...
def overlay_velo_img(img, velo_data,radius = 2):
    ...
    for i in range(0, len(x)):
        ...
        #print("zaxis % color %",z_axis[i],color)
        #cv2.circle(img, (x_axis[i], y_axis[i]), radius, [int(z_axis[i]*10), 50, 0],-1)
        #cv2.circle(img, (x_axis[i], y_axis[i]), radius, [color, color, color],-1)
        # not perfect but i think ok
        # mainly need a color scale of sorts
        #cv2.circle(img, (x_axis[i], y_axis[i]), int(color/256*velodyne_max_x/5), [color, color, color],0)
        #cv2.circle(img, (x_axis[i], y_axis[i]), radius, color,-1)
        #cv2.circle(img, (x_axis[i], y_axis[i]), radius, [10, 50, 0],-1)
