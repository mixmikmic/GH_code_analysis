...
get_ipython().run_line_magic('time', 'candidate_extractor.apply(train_docs, split=0, parallelism=PARALLEL)')

train_cands = session.query(Part_Attr).filter(Part_Attr.split == 0).all()
print("Number of candidates:", len(train_cands))

get_ipython().run_cell_magic('time', '', 'for i, docs in enumerate([dev_docs, test_docs]):\n    candidate_extractor.apply(docs, split=i+1)\n    print("Number of candidates:", session.query(Part_Attr).filter(Part_Attr.split == i+1).count())')

from fonduer import BatchFeatureAnnotator

featurizer = BatchFeatureAnnotator(Part_Attr)
get_ipython().run_line_magic('time', 'F_train = featurizer.apply(split=0, replace_key_set=True, parallelism=PARALLEL)')
print(F_train.shape)
get_ipython().run_line_magic('time', 'F_dev = featurizer.apply(split=1, replace_key_set=False, parallelism=PARALLEL)')
print(F_dev.shape)
get_ipython().run_line_magic('time', 'F_test = featurizer.apply(split=2, replace_key_set=False, parallelism=PARALLEL)')
print(F_test.shape)
...
get_ipython().run_line_magic('time', 'L_train = labeler.apply(split=0, clear=True, parallelism=PARALLEL)')
print(L_train.shape)
...
get_ipython().run_line_magic('time', 'L_train.lf_stats(L_gold_train)')

from fonduer import GenerativeModel

gen_model = GenerativeModel()
get_ipython().run_line_magic('time', 'gen_model.train(L_train, epochs=500, decay=0.9, step_size=0.001/L_train.shape[0], reg_param=0)')
...
get_ipython().run_line_magic('time', 'disc_model.train(F_train, train_marginals, n_epochs=200, lr=0.001)')
...
get_ipython().run_line_magic('time', '(TP, FP, FN) = entity_level_f1(true_pred, gold_file, ATTRIBUTE, test_docs, parts_by_doc=parts_by_doc)')
...
get_ipython().run_line_magic('time', 'L_train = labeler.apply(split=0, clear=False, update_keys=True, update_values=True, parallelism=PARALLEL)')
print(L_train.shape)
