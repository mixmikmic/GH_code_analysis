...
model_count = poisson_mod = sm.GLM(endog = Y, exog = X, family = sm.families.Poisson(),                                  offset = offsetx , freq_weights = weights).fit()
...
while np.absolute((ll_old - ll_new)/ll_old) > reltol :
    ...
    model_zero = sm.GLM(Y0.astype(int), exog = Z, family=sm.families.Binomial(),                        offset = offsetz, freq_weights = weights,                         start_params = start['zero']).fit()
    start = {'zero':model_zero.params, 'count':model_count.params}
...
def gradPoisson(parms):
    ...
    return (np.hstack((np.expand_dims(wres_count*weights,axis=1)*X,                 np.expand_dims(wres_zero*weights,axis=1)*Z))).sum(axis=0)
