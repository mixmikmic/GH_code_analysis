...
kwametensor = tract.generate_FSL_and_DTK_structure_tensor(format_img, filename = 's275minitensor', dogsigmaArr=[2], gausigmaArr=[4], angleArr=[25])

np.save('finaltensorimage',kwametensor[0])

tensorimg = kwametensor[0]
...
np.save('crucial', data)

sphere = get_sphere('symmetric724')
peak_indices = quantize_evecs(evecs, sphere.vertices)
eu = EuDX(im, peak_indices, seeds=50000, odf_vertices = sphere.vertices, a_low=0.2)
tensor_streamlines = [streamline for streamline in eu]

np.savez('crucial', streamlines)
...
labels = nib.load('ara.img').get_data()
...
get_ipython().system('python multigraphs.py crucial.npz albert/ ara.img')
