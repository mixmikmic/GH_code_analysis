...
class DataReader:
    ...
    def __read_samples(self, num_samples):
        labels = np.zeros((num_samples, self.num_docs))
        samples = []
        meta = []
        mb_size = 0
    ...
    def __get_interaction_features(self, samples):
        features = np.zeros((len(samples), self.num_docs, self.max_query_words, self.max_doc_words))
                ...
                for qw_idx, qword in enumerate(sample.query.split()):
                    ...
                    for dw_idx, dword in enumerate(doc.split()):
                        if dw_idx == self.max_doc_words:
                            break
                        if qword == dword:
                            features[sample_idx, doc_idx, qw_idx, dw_idx] = 1
        return features
        
    def __get_ngraph_features(self, samples):
        features_query = np.zeros((len(samples), self.num_ngraphs, self.max_query_words))
        features_docs = np.zeros((len(samples), self.num_docs, self.num_ngraphs, self.max_doc_words))
        for sample_idx, sample in enumerate(samples):
...
def duet(features_local, features_distrib_query, features_distrib_docs, num_ngraphs, words_per_query, words_per_doc, num_docs):
    ...
    duet_local    = C.Sequential ([
                        C.Convolution((1, words_per_doc), num_hidden_nodes, activation=C.tanh, strides=(1, 1), pad=False),
                        C.Dense(num_hidden_nodes, activation=C.tanh),
                        C.Dense(num_hidden_nodes, activation=C.tanh),
                        C.Dropout(prob=0.2),
                        C.Dense(1, activation=C.tanh)])
                        
    duet_embed_q  = C.Sequential ([
                        C.Convolution((word_window_size, 1), num_hidden_nodes, activation=C.tanh, strides=(1, 1), pad=False),
                        C.MaxPooling((pooling_kernel_width_query, 1), strides=(1, 1), pad=False),
                        C.Dense(num_hidden_nodes, activation=C.tanh)])
                        
    duet_embed_d  = C.Sequential ([
                        C.Convolution((word_window_size, 1), num_hidden_nodes, activation=C.tanh, strides=(1, 1), pad=False),
                        C.MaxPooling((pooling_kernel_width_doc, 1), strides=(1, 1), pad=False),
                        C.Convolution((1, 1), num_hidden_nodes, activation=C.tanh, strides=(1, 1), pad=False)])
                        
    duet_distrib  = C.Sequential ([
                        C.Dense(num_hidden_nodes, activation=C.tanh),
                        C.Dense(num_hidden_nodes, activation=C.tanh),
                        C.Dropout(prob=0.2),
                        C.Dense(1, activation=C.tanh)])
...
def train(train_file, ngraphs_file, num_docs, num_meta_cols):
    ...
    trainer = C.Trainer(z, ce, pe, [C.sgd(z.parameters, lr=lr_per_minibatch)])
