...
knn_accs, knn_pred, knn_pred_times, knn_fit_times = setup_dataset.run_knn(X_train, X_test, y_train, y_test, knn_c)

setup_dataset.compute_cm(y_test, knn_pred, knn_c)

knn_accs_scaled, knn_pred_scaled, knn_pred_times_scaled, knn_fit_times_scaled =setup_dataset.run_knn(X_train_scaled, X_test_scaled, y_train, y_test, knn_c)
...
dtc_accs, dtc_pred, dtc_pred_times, dtc_fit_times = setup_dataset.run_decision_tree(X_train, X_test, y_train, y_test, dtc_c)

dtc_accs, dtc_pred, dtc_pred_times, dtc_fit_times = setup_dataset.run_decision_tree(X_train, X_test, y_train, y_test, dtc_c)

dtc_accs_scaled, dtc_pred_scaled, dtc_pred_times_scaled, dtc_fit_times_scaled = setup_dataset.run_decision_tree(X_train_scaled, X_test_scaled, y_train, y_test, dtc_c)

setup_dataset.compute_cm(y_test, dtc_pred, dtc_c)

setup_dataset.compute_cm(y_test, dtc_pred_scaled, dtc_c)

nbc_accs, nbc_pred, nbc_pred_times, nbc_fit_times = setup_dataset.run_naive_bayes(X_train, X_test, y_train, y_test, (1,))

nbc_accs_scaled, nbc_pred_scaled, nbc_pred_times_scaled, nbc_fit_times_scaled = setup_dataset.run_naive_bayes(X_train_scaled, X_test_scaled, y_train, y_test, (1,))

setup_dataset.compute_cm(y_test, nbc_pred, [1])

setup_dataset.compute_cm(y_test, nbc_pred_scaled, [1])

abc_accs, abc_pred, abc_pred_times, abc_fit_times = setup_dataset.run_adaboost(X_train, X_test, y_train, y_test, (1,))

abc_accs_scaled, abc_pred_scaled, abc_pred_times_scaled, abc_fit_times_scaled = setup_dataset.run_adaboost(X_train_scaled, X_test_scaled, y_train, y_test, (1,))

setup_dataset.compute_cm(y_test, abc_pred, [1])

setup_dataset.compute_cm(y_test, abc_pred_scaled, [1])

qda_accs, qda_pred, qda_pred_times, qda_fit_times = setup_dataset.run_quadratic(X_train, X_test, y_train, y_test, (1,))

qda_accs_scaled, qda_pred_scaled, qda_pred_times_scaled, qda_fit_times_scaled = setup_dataset.run_quadratic(X_train_scaled, X_test_scaled, y_train, y_test, (1,))
...
svc_accs, svc_pred, svc_pred_times, svc_fit_times = setup_dataset.run_svc(X_train, X_test, y_train, y_test, svc_c)

svc_accs_scaled, svc_pred_scaled, svc_pred_times_scaled, svc_fit_times_scaled = setup_dataset.run_svc(X_train_scaled, X_test_scaled, y_train, y_test, svc_c)
...
rfc_accs, rfc_pred, rfc_pred_times, rfc_fit_times = setup_dataset.run_random_forest(X_train, X_test, y_train, y_test, rfc_c, rfc_jobs)

rfc_accs_scaled, rfc_pred_scaled, rfc_pred_times_scaled, rfc_fit_times_scaled = setup_dataset.run_random_forest(X_train_scaled, X_test_scaled, y_train, y_test, rfc_c, rfc_jobs)

setup_dataset.compute_cm(y_test, rfc_pred, rfc_c)

setup_dataset.compute_cm(y_test, rfc_pred_scaled, rfc_c)

gpc_accs, gpc_pred, gpc_pred_times, gpc_fit_times = setup_dataset.run_gaussian(X_train, X_test, y_train, y_test, gpc_c, gpc_jobs)

gpc_accs_scaled, gpc_pred_scaled, gpc_pred_times_scaled, gpc_fit_times_scaled = setup_dataset.run_gaussian(X_train_scaled, X_test_scaled, y_train, y_test, gpc_c, rfc_jobs)
