...
if not load_dict:
    ...
    # pickle.dump( dic, open( "../saved_models/exp_synthetic_rotating.p", "wb" ) ) # Uncomment for saving new model
else:
    dic = pickle.load( open( "./../saved_models/exp_synthetic_rotating.p", "rb" ) )

plot_laplacian(dic['X_true'], dic['Y_true'], 
               dic['lap_true'], dic['lap_inf'], 
               dic['xloc_syn'], dic['yloc_syn'], 
               dic['skl_div_syn'], T=[56,77,97,113,146],t_train = t_train, state = 'rot')
