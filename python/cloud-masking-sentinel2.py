...
# image collection
ic = ee.ImageCollection('COPERNICUS/S2')    .filterBounds(geom)    .filterDate(startDate,stopDate)
    
# single image
img = ee.Image(ic.first())

# top of atmosphere reflectance
toa = img.select(['B1','B2','B3','B4','B6','B8A','B9','B10', 'B11','B12'],                 ['aerosol', 'blue', 'green', 'red', 'red2','red4','h2o', 'cirrus','swir1', 'swir2'])                 .divide(10000).addBands(img.select('QA60'))                 .set('solar_azimuth',img.get('MEAN_SOLAR_AZIMUTH_ANGLE'))                 .set('solar_zenith',img.get('MEAN_SOLAR_ZENITH_ANGLE'))
