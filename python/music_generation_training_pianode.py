...
trainset = NotesGenerationDataset('../midi_data/Piano-midi.de/train/')
trainset.update_the_max_length()

trainset_loader = torch.utils.data.DataLoader(trainset, batch_size=20,
                                              shuffle=True, num_workers=4, drop_last=True)

valset = NotesGenerationDataset('../midi_data/Piano-midi.de/valid/')
...
rnn = RNN(input_size=88, hidden_size=1024, num_classes=88)
rnn = rnn.cuda()
...
for epoch_number in xrange(epochs_number):
    ...
    if current_val_loss < best_val_loss:
        ...
        torch.save(rnn.state_dict(), 'results/music_rnn_pianode.pth')
