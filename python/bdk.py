...
def DistilledSGLD(teacher_sym, student_sym,
    ...
    for i in xrange(total_iter_num):
        ...
        if (i + 1) % 2000 == 0:
            ...
            test_correct, test_total, test_acc =                 sample_test_acc(student_exe, X=X_test, Y=Y_test, label_num=10,
                                minibatch_size=minibatch_size)
            train_correct, train_total, train_acc =                 sample_test_acc(student_exe, X=X, Y=Y, label_num=10,
                                minibatch_size=minibatch_size)
            teacher_test_correct, teacher_test_total, teacher_test_acc =                 sample_test_acc(teacher_exe, X=X_test, Y=Y_test, label_num=10,
                                minibatch_size=minibatch_size)
            teacher_train_correct, teacher_train_total, teacher_train_acc =                 sample_test_acc(teacher_exe, X=X, Y=Y, label_num=10,
                                minibatch_size=minibatch_size)
            print "Student: Test %d/%d=%f, Train %d/%d=%f" % (test_correct, test_total, test_acc,
                                                       train_correct, train_total, train_acc)
            print "Teacher: Test %d/%d=%f, Train %d/%d=%f"                   % (teacher_test_correct, teacher_test_total, teacher_test_acc,
