...
nIter   = 1000
fc = np.array([25000, 75000, 100000, 150000])

# Create multiple sample irradiance patterns for various values of sigma_r
for currfc in fc:
    
    # Create a new collGrid with the updated fc
    collGrid = grids.Grid(20001, 5000, wavelength, currfc, dim = 1)   
    ...
    # Save the results
    np.save('x-coords0_fc_{0:d}.npy'.format(currfc), collGrid.px)
    np.save('avgIrrad0_fc_{0:d}.npy'.format(currfc), avgIrrad0)
    
    np.save('x-coords1_fc_{0:d}.npy'.format(currfc), grid.px)
    np.save('avgIrrad1_fc_{0:d}.npy'.format(currfc), avgIrrad1)
    
    np.save('x-coords2_fc_{0:d}.npy'.format(currfc), newGrid.px)
    np.save('avgIrrad2_fc_{0:d}.npy'.format(currfc), avgIrrad2)
    
    np.save('x-coords3_fc_{0:d}.npy'.format(currfc), newGrid.px)
    np.save('avgIrrad3_fc_{0:d}.npy'.format(currfc), avgIrrad3)
    
    np.save('x-coords4_fc_{0:d}.npy'.format(currfc), newGrid.pX)
    np.save('avgIrrad4_fc_{0:d}.npy'.format(currfc), avgIrrad4)
