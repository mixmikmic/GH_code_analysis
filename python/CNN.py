...
def CNN(la,shape=[None,None,None,None]):
    with tf.variable_scope('CNN'):
        w=weights_xavi(shape,xavier_params=(shape[-2],shape[-1]))
        b=weights_xavi([shape[-1],],xavier_params=(0,shape[-1]))
        act=tf.add(conv2d(la,w),b)
        tf.summary.histogram("weights", w)
        tf.summary.histogram("biases", b)
        tf.summary.histogram("activations", act)
        return act
...
def network(data):
    data=tf.reshape(data,[-1,28,28,1])  #makes the 784 col to a 28*28*1 image.
    l1=CNN(data,shape=[5,5,1,16])           
    l1=maxpool2d(l1)
    l1=tf.nn.relu(l1)
    l2=CNN(l1,shape=[5,5,16,32])
    l2=tf.nn.relu(l2)
    l2=maxpool2d(l2)
    l2=tf.contrib.layers.flatten(l2)                
    l3=neuralnet(l2,shape=[int(l2.shape[1]),100])
    l3=tf.nn.relu(l3)                          
    l4=neuralnet(l3,shape=[100,10])
    return l4   
