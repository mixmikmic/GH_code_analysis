...
AR = pd.read_csv('../../beast/main/discreteTraits/results/discreteTraits_2016-10-25_actualRates.csv', index_col=0, header=0).to_dict(orient='index')
BF = pd.read_csv('../../beast/main/discreteTraits/results/discreteTraits_2016-10-25_bf.csv', index_col=0, header=0).to_dict(orient='index')

prunedAR = pd.read_csv('../../beast/pruned/discreteTraits/results/pruned_combined_10mBurnin_actualRates.csv', index_col=0, header=0).to_dict(orient='index')
prunedBF = pd.read_csv('../../beast/pruned/discreteTraits/results/pruned_combined_10mBurnin_bf.csv', index_col=0, header=0).to_dict(orient='index')
...
# prunedtransmissions = defaultdict(list) # {from: [(to, bf, ar), .... ], ...}
# for from_host, to_dict in prunedBF.iteritems():
#     for to_host, bf in to_dict.iteritems():
#         if bf >= cutoff*12: # Account for the 12 segments' statistical nonindependence.
#             prunedtransmissions[from_host].append((to_host, bf, prunedAR[from_host][to_host]))
#         else:
#             continue


main_transmissions = []
pruned_transmissions = []
for from_host, to_dict in BF.iteritems():
    for to_host, bf in to_dict.iteritems():
        if bf >= cutoff*12: # Account for the 12 segments' statistical nonindependence.
            main_transmissions.append((from_host, to_host))
        else:
            continue
...
for from_host, to_dict in prunedBF.iteritems():
    for to_host, bf in to_dict.iteritems():
        if bf >= cutoff*12: # Account for the 12 segments' statistical nonindependence.
            pruned_transmissions.append((from_host, to_host))
        else:
            continue
            
main_only = set(main_transmissions).difference(set(pruned_transmissions))
pruned_only = set(pruned_transmissions).difference(set(main_transmissions))
shared_transmissions = set(main_transmissions).intersection(set(pruned_transmissions))
...
for (from_host, to_host) in shared_transmissions:
    c = 'gray'
...
for (from_host, to_host) in main_only:
    c = 'red'
    x, y = nodecoordinates[from_host]
    to_x, to_y = nodecoordinates[to_host]
    w = 6.
...
for (from_host, to_host) in pruned_only:
    c = 'green'
    x, y = nodecoordinates[from_host]
    to_x, to_y = nodecoordinates[to_host]
    w = 6.
...
# plt.savefig('../png/Fig3.png', dpi=600, bbox_inches='tight')
