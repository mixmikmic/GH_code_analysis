...
# From notebook 14 L(ZOGY) data for image with spatially varying PSF: (im1, im2, im1_psf, im2_psf, conv_im1, pci, pcf)
# Note for this to be correct, #14 needs to be run without a bulk astrometric offset (default is 0.2, set it to zero!)
LZOGY = cPickle.load(gzip.GzipFile("14_results.p.gz", "rb"))
...
plt.imshow(diffim/diffim.std(), origin='lower', interpolation='none', cmap='gray', clim=(-5,5))
plt.colorbar()
...
plt.imshow(cov, interpolation='none', clim=(0, 1.))
plt.colorbar()
tmp2 = cov.copy()
