...
get_ipython().magic('time (X_train, y_train), (X_test, y_test) = mnist.load_data()')
...
get_ipython().magic('time P = compute_joint_probabilities(X_train, batch_size=batch_size, verbose=2)')

get_ipython().magic("time np.save('P.npy', P)")
...
model.add(Dense(2))
sgd = SGD(lr=0.1)
get_ipython().magic('time model.compile(loss=tsne, optimizer=sgd)')
...
get_ipython().magic('time model.fit(X_train, Y_train, batch_size=batch_size, shuffle=False, nb_epoch=100)')
...
get_ipython().magic('time X_2d = bh_sne(X_train.astype(np.float64))')

get_ipython().magic("time np.save('X_2d.npy', X_2d)")
...
encoder.add(Dense(2))
get_ipython().magic("time encoder.compile(loss='mse', optimizer='rmsprop')")

get_ipython().magic('time encoder.fit(X_train, X_2d, nb_epoch=100, verbose=2)')
...
plot_differences(encoder.predict(X_train), model.predict(X_train))
...
nb_conv = 3
conv = Sequential()
conv.add(Convolution2D(nb_filters, nb_conv, nb_conv, input_shape=(28, 28, 1)))
...
conv.add(Activation('relu'))
conv.add(Dense(2))
get_ipython().magic("time conv.compile(loss='mean_squared_error', optimizer='rmsprop')")

X_train_conv = X_train.reshape(-1, 28, 28, 1)
get_ipython().magic('time conv.fit(X_train_conv, X_2d, nb_epoch=100)')

plot_model(conv.predict(X_train_conv), y_train)

plot_differences(conv.predict(X_train_conv), X_2d)
...
get_ipython().magic("time decoder.compile(loss='mse', optimizer='rmsprop')")

get_ipython().magic('time decoder.fit(X_2d, X_train, nb_epoch=100, verbose=2)')
...
get_ipython().magic("time ae.compile(loss='mse', optimizer='rmsprop')")

get_ipython().magic('time ae.fit(X_train, X_train, nb_epoch=100, verbose=2, batch_size=32)')
...
get_ipython().magic("time ae_decoder.compile(loss='mse', optimizer='rmsprop')")
...
get_ipython().magic("time ae_scratch.compile(loss='mse', optimizer='rmsprop')")
...
get_ipython().magic("time ae_scratch_decoder.compile(loss='mse', optimizer='rmsprop')")
