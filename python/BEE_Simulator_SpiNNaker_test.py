...
#         spikes.append(spikes_idx[spks])
    ...
    exc_inputs = range(NofIN)
    inh_inputs = []
    exc_weights = [AINP*1E-9]*NofIN
    inh_weights = []
    updates_sim(exc_inputs, inh_inputs, exc_weights, inh_weights,NofIN,0)
...
net_shape = numpy.array([15,30,30])
NofN = net_shape.prod()
if not BEE_initialized():
    get_ipython().magic('time initialize_sim(my_net_shape = net_shape, my_lbd_value = 1.2,                          my_seeds=seeds, SpkLiq_vresets = [13.5E-3,13.5E-3+1E-12])#, SpkLiq_noisy_current_rand = 0.2E-3)')
...
#
# SAVES THE INDICES OF THE EXCITATORY NEURONS
#
numpy.save("exc_neuron_idx",output_exc_indices(NoEXCN))
output_exc_indices(NoEXCN)

#
# SAVES THE INDICES OF THE INHIBITORY NEURONS
#
numpy.save("inh_neuron_idx",output_inh_indices(NoINHN))
output_inh_indices(NoINHN)
...
#
# SAVES THE NUMBER OF NEURONS
#
numpy.save("Number_of_neurons_lsm",Number_of_neurons_lsm)
...
#
# SAVES THE REFRACTORY VECTOR
#
numpy.save("refractory_vector",refractory_vector)
...
# SAVES THE I_OFFSET
#
numpy.save("i_offset",pop_lsm.i_offset)
...
#
# SAVES THE INITIAL MEMBRANE VOLTAGE
#
numpy.save("init_membrane_v",pop_lsm.v)
...
#
# SAVES THE INDICES_PRE_EXC
#
numpy.save("indices_pre_exc",output_pre_e_connections(NoEXCC))


#
# SAVES THE INDICES_POS_EXC
#
numpy.save("indices_pos_exc",output_pos_e_connections(NoEXCC))

#
# SAVES THE WEIGHT_PRE_EXC
#
numpy.save("weights_pre_exc",(output_pre_e_weights(NoEXCC)*1E9).astype(numpy.double))
...
#
# SAVES THE INDICES_PRE_INH
#
numpy.save("indices_pre_inh",output_pre_i_connections(NoINHC))


#
# SAVES THE INDICES_POS_EXC
#
numpy.save("indices_pos_inh",output_pos_i_connections(NoINHC))

#
# SAVES THE WEIGHT_PRE_EXC
#
numpy.save("weights_pre_inh",(output_pre_i_weights(NoINHC)*1E9).astype(numpy.double))
