...
class Firm(object):
    def __init__(self, inertia=0, skepticism=0):
        ...
        # The model is initialized in a state such that the first choice has been made, 
        # but the first outcome has not been assessed
    ...
    def decision(self, winning_innovation, n_winners_made):
        ...
        if np.random.binomial(1, p_discard) == 1:  # discard the current choice
            ...
            time_since_last_tried = list(reversed(self.choice_history)).index(winning_innovation) if                                     winning_innovation in self.choice_history else                                     100000  # called t_star(w)
            
            influence_of_internal_abandonment = 1 if time_since_last_tried > consideration_interval else                                             (1 - self.skepticism**(lam * time_since_last_tried))
...
def run(firms, n_rounds=200):
    ...
    for i in range(1, n_rounds):
        ...
        considered_winning_uses = [i for i in winning_uses_of_yesterdays_winner 
                                   if i > t-consideration_interval]
        ...
        use.append(pd.value_counts(decisions))
        
    result = {
        # popularity counts the average number of firms using the leading 
        # innovation in each iteration
        'popularity': popularity_history,
        # turnover counts the number of changes in the identity of the leading 
        # innovation per 100 iterations
        'turnover': sum(np.diff(most_popular_history) != 0)/(n_rounds/100),
        'total use': pd.DataFrame(use),
    }
...
result['total use'].plot(figsize=(12,4), alpha=.6, marker='.')
plt.ylabel('Popularity')
plt.xlabel('Steps');

result['total use'][[0,1]].plot(figsize=(12,4), alpha=.6, marker='.')
plt.ylabel('Popularity')
plt.xlabel('Steps');

result['total use'].plot(figsize=(12,4), alpha=.6, kind='area')
plt.ylabel('Popularity')
plt.xlabel('Steps');

bias = .95
selections = np.random.binomial(1, .5, size=1000)
adoption_history = [np.mean(selections)]
for i in range(200):
    winner = int(np.random.uniform(0,1000))
    winner_selection = selections[winner]
    p = bias if winner_selection == 1 else 1-bias
    selections = np.random.binomial(1, p, size=1000)
    adoption_history.append(np.mean(selections))
    
plt.plot(adoption_history)
plt.ylabel('Fraction Adopting');
