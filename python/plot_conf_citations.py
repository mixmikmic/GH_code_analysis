...
plot_dir = '/localdata/u5642715/influenceMapOut/img/citation' #'./plots'
post_dir = '/localdata/u5642715/influenceMapOut/content/citation'

if not os.path.exists(os.path.join(plot_dir, conf)):
    os.makedirs(os.path.join(plot_dir, conf))

if not os.path.exists(os.path.join(post_dir, conf)):
    os.makedirs(os.path.join(post_dir, conf))

d = pickle.load( open(os.path.join(out_dir, conf, 'cite_records.'+conf+".pkl"), 'rb') )
...
x_pos = [x+1 for x in range(len(citing_not_null))]
print(citing_not_null, citing_num_null)
print(len(citing_not_null), len(citing_num_null), len(x_pos))
...
ax.xaxis.tick_top()
ax.invert_yaxis()
ax.set_ylabel('reference-published-year')
ax.set_xlabel(conf + ' paper-published-year')
# sns.axlabel(conf + ' paper-published-year', 'reference-published-year')
...
ax.set_ylabel('citation-published-year')
ax.set_xlabel(conf + ' paper-published-year')
ax.xaxis.tick_top()
ax.invert_yaxis()
...
ax = sns.lmplot(x='gap', y='cite_rate', data=df_survive, 
                line_kws={'c':'0.5', 'lw':1, 'ls':'solid'}) #, scatter_kws={'c':bar_colrs2[mdx]}) Doesn't work?
...
# print(venue_ref.items())
nx.set_node_attributes(venueG, venue_ref, 'citation_out')
nx.set_node_attributes(venueG, venue_citation, 'citation_in')
nx.set_node_attributes(venueG, venue_metric, 'citation_flow')
...
ax.xaxis.tick_top()
ax.set_ylabel('reference-venue')
ax.set_xlabel('publication year of ' + conf + ' paper')
...
print(os.path.join(plot_dir, 'fig_text.json'))
