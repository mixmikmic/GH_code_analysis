...
def simulated_annealing(problem, schedule, value_method="L2"):
    ...
    while True:
        T = schedule(t)
        if T < 1e-10: return current
        successor = random.choice(problem.successors(), 1)[0]
        delta = successor.get_value(value_method) - current.get_value(value_method)
...
class TravelingSalesmanProblem:
    ...
    def successors(self):
        ...
        """
        n = len(self.path)
        successors = [TravelingSalesmanProblem(self.path[-1:-2:-1]+self.path[1:-1]+self.path[:1])]
        for i in range(n-1):
            successors.append(TravelingSalesmanProblem(self.path[:i] + self.path[i:(i+2)][::-1] + self.path[(i+2):]))
        return successors

    def get_value(self, method="L2"):
