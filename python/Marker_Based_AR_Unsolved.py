...
# Here we define a axis representing object points which we will plot in image space 
axis = np.float32([[3,0,0], [0,3,0], [0,0,-3]]).reshape(-1,3)

img = cv2.imread(os.path.join(PATTERN_PATH,"left12.jpg"))
gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)

# Finding the corners in a Chessboard using findChessBoardCorners
ret, corners = cv2.findChessboardCorners(gray, (7,6),None)

if ret == True:
    corners2 = cv2.cornerSubPix(gray,corners,(11,11),(-1,-1),criteria)
    
    # Use solvePnP to estimate the object pose given a set of object and image points
    ret, rvecs, tvecs = cv2.solvePnP(obj_points, corners2, mtx, dist)
    
    # projecting 3D points to image plane using projectPoints
    imgPts, jac = cv2.projectPoints(axis, rvecs, tvecs, mtx, dist)
    
    # using the draw helper function to draw the projected input points
    img = draw(img, corner2, imgPts)
