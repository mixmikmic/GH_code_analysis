...
reload(dit)
breakLimit = 1.020  # break limit is actually 1.05 but this is really bad in crowded fields
...
P_n = testObj.im2.psf
testObj.im2.psf = dit.makePsf(psfSize=13, sigma=[1.8*breakLimit,2.2*breakLimit], theta=-45.*breakLimit)
print np.sqrt(testObj.astrometricOffsets)
...
def runTest(flux, seed=66, sky=300., n_sources=2500, n_varSources=1, percentagePsfMismeasure=0., scint=0.01):
    try:
...
print len(inputs)
testResults1 = Parallel(n_jobs=num_cores, verbose=2)(delayed(runTest)(i[0], i[1], n_sources=n_sources, n_varSources=10,                                                                       percentagePsfMismeasure=i[2], scint=0.01)                                                      for i in inputs)
...
inputs = [(f, seed, ppm) for f in [1500] for seed in np.arange(66, 166, 1) for ppm in errorVals]
print len(inputs)
testResults2 = Parallel(n_jobs=num_cores, verbose=2)(delayed(runTest)(i[0], i[1], n_sources=100, n_varSources=10,                                                                       percentagePsfMismeasure=i[2], scint=0.005)                                                      for i in inputs)
